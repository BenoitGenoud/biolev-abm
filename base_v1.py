...
class PrimateAgent:
    def __init__(self, id, parent=None):
        ...
        if use_variable_altruism:
            self.altruism = parent.altruism if parent and hasattr(parent, "altruism") else random.uniform(0.3, 0.7)

        if use_kin_selection:
            self.family_id = parent.family_id if parent else random.randint(0, 10)  # 10 familles max

        if use_noise:
            self.error_rate = 0.05  # erreur aléatoire de stratégie
